...
def start_photo_process(selected_plant, x, y, distance):
    ...
    def update_progress():
        ...
        latest_image = None
        images_response = requests.get(farmbot_url, headers=farmbot_headers)
        ...
        if latest_image:
            ...
            Label(image_window, text="Latest Image").pack()
            latest_image_label = Label(image_window, image=image)
            latest_image_label.image = image  # Keep a reference to avoid garbage collection
            latest_image_label.pack()

            # Add a button to download the latest image
            download_button_latest = ctk.CTkButton(image_window, text="Download Latest Photo",
                                                   command=lambda: download_photo(latest_image_url))
            download_button_latest.pack(pady=10)
