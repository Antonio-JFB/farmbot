...
def show_plant_details(selected_plant):
    ...
    ctk.CTkLabel(details_frame, text=f"Scientific Name: {selected_plant['Scientific Name']}", anchor=W).pack(pady=2, anchor=W)
    ctk.CTkLabel(details_frame, text=f"Start Date: {selected_plant['Start Date']}", anchor=W).pack(pady=2, anchor=W)
    ctk.CTkLabel(details_frame, text=f"FarmBot Coordinates: {selected_plant['FarmBot Coordinates']}", anchor=W).pack(pady=2, anchor=W)
    ...
    ctk.CTkLabel(details_frame, text=f"Seed Bag IDs: {selected_plant.get('Seed Bag IDs')}", anchor=W).pack(pady=2, anchor=W)
    ...
    if related_events:
        ctk.CTkLabel(details_frame, text="Related Events:", anchor=W).pack(pady=10, anchor=W)
        for event in related_events:
            ...
            event_label = ctk.CTkLabel(details_frame, text=f"Event: {start_dt} to {end_dt} ({duration_weeks} weeks)", anchor=W)
            event_label.pack(pady=2, anchor=W)
            delete_button = ctk.CTkButton(details_frame, text="Delete Event",
                                          command=lambda e=event['id']: delete_farm_event(e))
            delete_button.pack(pady=2)
    else:
        ctk.CTkLabel(details_frame, text="No related events found.", anchor=W).pack(pady=10, anchor=W)
...
def prompt_moisture_level(selected_plant, x, y):
    ...
    label_moisture = ctk.CTkLabel(frame, text="Enter the moisture level (0-100):", text_color="white")
    label_moisture.pack(pady=(10, 5))

    # Entry for moisture level
    moisture_var = StringVar()
    moisture_entry = ctk.CTkEntry(frame, textvariable=moisture_var)
    moisture_entry.pack(pady=(0, 10))

    # Label for duration
    label_duration = ctk.CTkLabel(frame, text="Enter duration in weeks:", text_color="white")
    label_duration.pack(pady=(10, 5))
...
def create_farm_event(selected_plant, x, y, moisture_level, duration_weeks):
    ...
    now = datetime.utcnow()
    start_time = now.replace(hour=16, minute=30, second=0, microsecond=0).isoformat() + "Z"
    end_time = (now + timedelta(weeks=duration_weeks)).replace(hour=16, minute=30, second=0, microsecond=0).isoformat() + "Z"
